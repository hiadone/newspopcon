...
$(document).ready(function(){
	...
	$.ajax({
        type: "GET", 
        async: true,
        data: "pageid=058W&lang=utf-8&out=json", 
        ...
        success: function(data) 
        {
		 ...
		 $("#foin_058W").html(data.tag).promise().done(function(){
      ...
      infiniteLoop: true,//마지막에 도달 했을시, 첫페이지로 갈 것인가 멈출것인가
	   ...
	   onSliderLoad: function(){
		     setTimeout(function() {
				$('#sliders', parent.document).height($(".slide").height()+10);
				  }, 30);
			  if($(".slide").height() <50 ){
				  $('#sliders', parent.document).height(190);		
...
if($(".bx-viewport").height() < 35) {
	//$(".bx-viewport").css('height',iframeHeight);
     console.log($('#sliders', parent.document).height() +'viewport' );
}
//$('#sliders', parent.document).height($(".thum a img").height());

// console.log($("#foin_058W .thum a img").height()+'silder');
        },
